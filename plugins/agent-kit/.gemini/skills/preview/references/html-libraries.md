<!-- Adapted from ck:preview v1.1.0 (plugins/agent-kit/skills/ck-preview). Cloned for ak:preview to stay decoupled; keep in sync manually if ck-preview improves. -->
...
### Render guard (validate before run, re-theme on toggle)

A single malformed diagram otherwise renders Mermaid's error "bomb" SVG. Always validate with `mermaid.parse()` before `mermaid.run()` and degrade to a readable code block on failure. Expose the render function so the theme toggle can re-render (Mermaid can't switch theme reactively).

```html
<pre class="mermaid" id="flow"> ...diagram source... </pre>
<script type="module">
import mermaid from 'https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs';
const flow = document.getElementById('flow');
const src = flow.textContent.trim();
const isDark = () => (document.documentElement.getAttribute('data-theme')
  || (matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')) === 'dark';
async function renderMermaid() {
  try {
    mermaid.initialize({ startOnLoad: false, securityLevel: 'loose', theme: isDark() ? 'dark' : 'neutral' });
    await mermaid.parse(src);                 // throws on syntax error
    flow.classList.add('mermaid');
    flow.removeAttribute('data-processed');
    flow.innerHTML = src;
    await mermaid.run({ nodes: [flow] });
  } catch (e) {                               // degrade to code; page still renders
    flow.classList.remove('mermaid');
    flow.innerHTML = '<pre class="mermaid-fallback"><span class="note">diagram source (render failed)</span></pre>';
    flow.querySelector('.mermaid-fallback').appendChild(document.createTextNode(src));
  }
}
window.__renderMermaid = renderMermaid;   // call again inside the theme-toggle handler
renderMermaid();
</script>
```
