...
### Phase 6 — Feed it back and re-run (Mode A only)
...
These close the bias gap from running producers in main context. They stop self-grading, moving goalposts, and invented verdicts.
...
- **`code-review`** runs after `validate` passes. The validator catches "wrong"; code-review catches "could be better". Same for `code-refactor` and `code-simplify`.
