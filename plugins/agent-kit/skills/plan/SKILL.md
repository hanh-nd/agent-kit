...
- **Slug Selection (MUST run before Step 4.1).** Parse `$ARGUMENTS` for any path segment matching `.agent-kit/handoffs/<slug>/` or `.agent-kit/handoffs/<slug>`. If found, extract `<slug>` verbatim — use it exactly as the plan slug. Do NOT append or substitute feature names, ticket titles, or version suffixes. Only if no handoffs path is present in `$ARGUMENTS` should you derive a slug from the feature or ticket name. Once chosen, freeze this slug. Every Phase 4 `kit_save_handoff` call MUST reuse it exactly. Do NOT rely on content-derived slug fallback, because each file has different content and could otherwise land in a different folder.
