...
| **Rebuilt helpers** | A new page object, fixture, selector helper, or login routine duplicating one already in the suite. Search the sibling specs and the shared test-utils directory by behavior, not name. A helper with one caller belongs in the test that uses it. |
